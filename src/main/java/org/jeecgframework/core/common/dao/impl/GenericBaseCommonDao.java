...
import org.jeecgframework.core.util.ThreadContext;
...
//		}
	...
	{
		String dbk = ThreadContext.getDbSource();
		JdbcTemplate jdbcTemplateNow = null;
		if(dbk!=null&&!"null".equals(dbk))
		{
			System.out.println(Thread.currentThread().getName()+"=======================================");
			jdbcTemplateNow=DynamicDBUtil.getJdbcTemplate(dbk);
			
		}else
		{
			jdbcTemplateNow = this.jdbcTemplate;
			
		}
		
		ThreadContext.setDbSource(null);
		
		return jdbcTemplateNow;
	}
	
	private NamedParameterJdbcTemplate getRealNamedParameterJdbcTemplate()
	{
		String dbk = ThreadContext.getDbSource();
		NamedParameterJdbcTemplate jdbcTemplateNow = this.namedParameterJdbcTemplate;
		if(dbk!=null)
		{
			jdbcTemplateNow=new NamedParameterJdbcTemplate(DynamicDBUtil.getJdbcTemplate(dbk));
	...
	public List<Map<String, Object>> findForJdbc(String sql, int page, int rows) {
		...
		JdbcTemplate jdbcTemplateNow = getRealJdbcTemplate();
		
		return jdbcTemplateNow.queryForList(sql);
	}
	...
	public <T> List<T> findObjForJdbc(String sql, int page, int rows,
		...
		sql = JdbcDao.jeecgCreatePageSql(sql, page, rows);
		JdbcTemplate jdbcTemplateNow = getRealJdbcTemplate();
		List<Map<String, Object>> mapList = jdbcTemplateNow.queryForList(sql);
	...
	public List<Map<String, Object>> findForJdbcParam(String sql, int page,
		...
		return jdbcTemplateNow.queryForList(sql, objs);
	}
	...
	public Long getCountForJdbc(String sql) {
		...
		JdbcTemplate jdbcTemplateNow = getRealJdbcTemplate();
		return jdbcTemplateNow.queryForObject(sql,Long.class);
	}
	...
	public Long getCountForJdbcParam(String sql, Object[] objs) {
		...
		JdbcTemplate jdbcTemplateNow = getRealJdbcTemplate();
		return jdbcTemplateNow.queryForObject(sql, objs,Long.class);
	...
	public Integer executeSql(String sql, List<Object> param) {
		JdbcTemplate jdbcTemplateNow = getRealJdbcTemplate();
		return jdbcTemplateNow.update(sql, param);
	}
	...
	public Integer executeSql(String sql, Map<String, Object> param) {
		...
		NamedParameterJdbcTemplate namedParameterJdbcTemplateNow = getRealNamedParameterJdbcTemplate();
		
		return namedParameterJdbcTemplateNow.update(sql, param);
	}
	public Object executeSqlReturnKey(final String sql, Map<String, Object> param) {
		...
		if (StringUtil.isNotEmpty(param.get("id"))) {//表示已经生成过id(UUID),则表示是非序列或数据库自增的形式
			namedParameterJdbcTemplateNow.update(sql,sqlp);
		//--author：zhoujf---start------date:20170216--------for:自定义表单保存数据格sqlserver报错问题
			...
			namedParameterJdbcTemplateNow.update(sql,sqlp, keyHolder, new String[]{"id"});
	...
	public Integer countByJdbc(String sql, Object... param) {
		JdbcTemplate jdbcTemplateNow = getRealJdbcTemplate();
		return jdbcTemplateNow.queryForObject(sql, param,Integer.class);
	...
	public Map<String, Object> findOneForJdbc(String sql, Object... objs) {
		try {
			JdbcTemplate jdbcTemplateNow = getRealJdbcTemplate();
			return jdbcTemplateNow.queryForMap(sql, objs);
